...
#     clf = NB_Text_Classifier(item_texts, item_categories)
#     clf.fit()
#
#     text = 'A brand new smartphone'
#     category_probs = clf.predict(text)
#
#     for category, prob in category_probs.items():
#         print(f'P({category}|{text}) = {prob:.5f}')
#
#     predicted_category = max(category_probs, key=category_probs.get)
#     print(f'Tekst spada u kategoriju: {predicted_category}')
#
...
#     df.columns = ['category', 'text']
#
#     # Zamenimo NaN vrednosti u koloni 'text' sa praznim stringovima
...
#
#     # Calculate accuracy
#     correct_predictions = sum(1 for true, pred in zip(test_categories, predictions) if true == pred)
#     accuracy = correct_predictions / len(test_categories)
#
